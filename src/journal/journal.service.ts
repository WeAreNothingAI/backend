...
export class JournalService {
  ...
  async handleUpload(file: Express.Multer.File): Promise<any> {
    // 1. S3 업로드
    const key = `${Date.now()}-${file.originalname}`;
    const s3Url = await this.s3Service.uploadAudio(file, key);

    // 2. Whisper STT 호출
    const result = await this.callWhisper(s3Url);

    await this.saveEditedTranscript(1, 1, s3Url, result);
  ...
  async callWhisper(audioUrl: string): Promise<any> {
    try {
      const res = await axios.post('http://localhost:5000/transcribe', {
        audio_url: audioUrl,
      });
      ...
      // 에러 로깅 및 예외 처리
      throw new Error('Whisper 호출 실패: ' + error.message);
    }
  }
  ...
  async saveEditedTranscript(
    ...
    return await this.prisma.journal.create({
      data: { careWorkerId, clientId, rawAudioUrl, transcript },
    });
  }
}
